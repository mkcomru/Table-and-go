...
class PasswordResetRequestView(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            send_password_reset_email.delay(email, user.id)
            
            return Response({
                "detail": "Инструкции по восстановлению пароля отправлены на ваш email."
            })
        except User.DoesNotExist:
            return Response({
                "detail": "Если указанный email зарегистрирован в системе, инструкции по восстановлению пароля будут отправлены."
            })
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if default_token_generator.check_token(user, token):
                ...
                return Response({
                    "valid": False,
                    "detail": "Ссылка для восстановления пароля недействительна или срок её действия истек."
                }, status=status.HTTP_400_BAD_REQUEST)
...
class PasswordResetCompleteView(views.APIView):
    ...
    def post(self, request):
        ...
        if not uidb64 or not token or not password:
            return Response({
                "detail": "Необходимо указать все обязательные параметры."
            }, status=status.HTTP_400_BAD_REQUEST)
